...
def getPlayList(request, play_list_id):
    ...
    if play_list_id == "all":
        aListOfPlays = PlayDAO.objects.all();
    else:
        aPlayList = get_object_or_404(PlayListDAO, url_title=play_list_id)
        aListOfPlays = [entry.play for entry in aPlayList.playlistentrydao_set.all()]

    aPlayDictList = [vars(Play(x)) for x in aListOfPlays]
